...
**Версия:** 1.7.1 + исправления выборочной очистки

## Исправления поведения

- «Очистить» обрабатывает только отмеченные строки после сканирования. Пустой выбор ничего не запускает.
- Нет скрытой глобальной очистки USBSTOR, VSS, дополнительных журналов и PnP-призраков.
- Симуляция включена по умолчанию; автоматическая перезагрузка выключена.
- Активные устройства защищены при удалении ветвей Enum и PnP-призраков; перед очисткой всё равно отключайте накопители.
- Ошибка обработки выбранного элемента означает неуспешный результат. Наличие невыбранных USB-записей само по себе не является ошибкой.
- Неполный экспорт выбранных ветвей реестра прерывает очистку, если включён бэкап. `.reg` не восстанавливает файлы и журналы событий.
- После реальной очистки GUI повторно сканирует систему.
- Обычный `dotnet test` больше не запускает разрушительный тест очистки USBSTOR.

Изменения относятся к основному USB-пути. Расширенные сканеры и отдельные вкладки требуют дополнительной проверки в виртуальной машине. Инженерное руководство в `docs/` описывает исходную 1.7.1; при расхождении ориентируйтесь на этот раздел.
...
- Критерий успеха: выбранные элементы обработаны без ошибок; общий счётчик USBSTOR справочный
...
- CLI: `--clean` — сканирование и обработка найденных элементов (без глобальной зачистки вне списка)
...
- VSS не удаляются в основном USB-пути
...
Сканирование и обработка всех найденных элементов. Без авто-перезагрузки и скрытых глобальных действий. Это реальная очистка, включая найденные файлы и журналы.
...
4. Удаление VSS отключено в основном USB-пути
