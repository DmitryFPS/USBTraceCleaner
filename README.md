...
1. Нажмите **«Найти устройства»** — сканирование не меняет Windows.
2. Найдите устройство по имени, пути или серийному номеру. Фильтр **«Только отключённые»** скрывает активные и неопределённые записи.
3. Нажмите строку: внизу показаны источник имени, последствия очистки и путь, который можно скопировать.
4. Отметьте ненужные записи и нажмите **«Проверить выбранное»**. Ничего не удаляется, план доступен в журнале.
5. Для удаления отключите накопители, снимите **«Проверка без удаления»**, оставьте резервную копию включённой и нажмите **«Удалить выбранное»**.

Одна строка — запись Windows, а не обязательно отдельное физическое устройство. Имена берутся из FriendlyName/DeviceDesc, связей экземпляра, ContainerID и драйвера. Для VID/PID показывается связь с моделью, без утверждения о конкретном экземпляре. При отсутствии достоверного имени это явно указано. Названия уже удалённых устройств не всегда можно восстановить.

Обычный поиск показывает записи USB-накопителей и MTP. История файлов, общие журналы и дополнительные ветви доступны через **«Дополнительные настройки» → «Расширенный поиск»**. После изменения настроек повторите сканирование. По умолчанию строки не отмечены; активные устройства недоступны для выбора. Type-C — тип разъёма: Thunderbolt/USB4/DisplayPort-устройства без USB-записей этот поиск не охватывает.

### Исправления 1.8.0

- Поиск старых PnP-записей теперь только показывает список; очистка выполняется общей кнопкой с учётом симуляции и резервной копии.
- «Другие USB-следы»: симуляция без администратора, имена/состояния, удаление только показанных путей; отключение живых устройств и изменение общих setupapi-логов убраны.
- Сетевой аудит: симуляция по умолчанию, пароли скрыты, нет чужого белого списка; глобальные действия вне найденного списка убраны. Ошибки внешних команд и удаления реестра больше не считаются успехом.
- Исправлены счётчики симуляции, потеря результата сетевой очистки после повторного сканирования, необработанные ошибки и одновременный запуск действий на разных вкладках.
- Во вкладке журналов больше нет дополнительного удаления System.evtx. Очищаются только выбранные каналы штатным способом.
- После работы с SRU восстанавливаются только службы, которые остановила программа; исправлены утечки дескрипторов реестра.
- В PDF добавлены имена устройств, состояние и последствия удаления.

Проверено: Release build/publish, автоматические тесты и создание WPF-окна с демонстрационными записями (поиск, фильтр, блокировка активного устройства, блокировка действий во время работы). Реальное удаление не запускалось. Проверка под администратором на Windows 11 и разных USB/UASP/MTP-устройствах остаётся необходимой; тесты с ранним выходом без прав не подтверждают эти сценарии. Старое инженерное руководство в docs описывает исходную версию, а не новый интерфейс.
